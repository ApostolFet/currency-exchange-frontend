$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.each(data, function (index, currency) {
          const row = $("<tr></tr>");
          row.append($("<td id='currency-code'></td>").text(currency.code));
          row.append($("<td></td>").text(currency.name));
          row.append($("<td></td>").text(currency.sign));
          row.append(
            $("<td></td>").append(
              $(
                '<button type="button" class="btn btn-danger btn-sm" id="delete-currency">Delete</button>',
              ),
            ),
          );
          tbody.append(row);
  ...
  $(document).delegate(".exchange-rate-edit", "click", function () {
    ...
    $("#edit-exchange-rate-modal .modal-title").text(
      `Edit ${pair} Exchange Rate`,
    );
  ...
  $("#edit-exchange-rate-modal .btn-primary").click(function () {
    ...
    const pair = $("#edit-exchange-rate-modal .modal-title")
      .text()
      .replace("Edit ", "")
      .replace(" Exchange Rate", "");
    const exchangeRate = $(
      "#edit-exchange-rate-modal #exchange-rate-input",
    ).val();
  ...
  $(document).on("click", "#delete-currency", function () {
    const currentElem = $(this).closest("tr");
    const currency = currentElem.find("td#currency-code:first").text();

    $.ajax({
      url: `${host}/currency/${currency}`,
      type: `DELETE`,
      success: function () {
        currentElem.remove();
        $(".exchange-rates-table")
          .find("tr")
          .each(function () {
            const pair = $(this).find("td:first").text();
            if (pair.startsWith(currency) || pair.endsWith(currency)) {
              $(this).remove();
            }
          });
      },
